...
int main() {
  ...
  h.onMessage([&lane, &ref_velocity, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // Create a list of widely spaced anker waypoints
            vector<double> ptsx;
            vector<double> ptsy;
            ...
            // if previous size becomes empty, use car as starting reference
            if (prev_size < 2)
            {
                // no previous path points:
            ...
            // create list of next waypoints and push previous path
            ...
            // calculate waypoints using splines and considering the reference velocity
            ...
            // fill up rest of the path planers path
            for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
                double N       = target_dist / ( time_step * ref_velocity / 2.24 );
